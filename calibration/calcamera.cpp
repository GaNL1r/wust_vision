...
void load(
    ...
    for (int i = 1; true; i++) {
        ...
        auto success =
            cv::findChessboardCorners(img, pattern_size, centers_2d, cv::CALIB_CB_SYMMETRIC_GRID);
...
void print_yaml(const cv::Mat& camera_matrix, const cv::Mat& distort_coeffs, double error) {
    ...
    std::vector<double> camera_matrix_data(
        camera_matrix.begin<double>(),
        camera_matrix.end<double>()
    );
    std::vector<double> distort_coeffs_data(
        distort_coeffs.begin<double>(),
        distort_coeffs.end<double>()
    );
...
int main(int argc, char* argv[]) {
    ...
    std::string input_folder = "test";
    std::string config_path = "/home/hy/wust_vision/config/cal.yaml";

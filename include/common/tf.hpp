...
public:
    ...
    void getRotation(Quaternion& q) const {
        ...
        if (trace > 0.0) {
            ...
            int i =
                (m[0][0] > m[1][1]) ? ((m[0][0] > m[2][2]) ? 0 : 2) : ((m[1][1] > m[2][2]) ? 1 : 2);
            double s;
            if (i == 0) {
...
struct Transform {
    ...
    Eigen::Isometry3d toEigenIso() const {
        ...
        iso.rotate(Eigen::Quaterniond(orientation.w, orientation.x, orientation.y, orientation.z));
        return iso;
    }

    static Transform
    fromEigen(const Eigen::Isometry3d& iso, std::chrono::steady_clock::time_point stamp) {
        Eigen::Vector3d pos = iso.translation();
        Eigen::Quaterniond q(iso.rotation());
        return { { pos.x(), pos.y(), pos.z() }, Quaternion(q.x(), q.y(), q.z(), q.w()), stamp };
    }
};
            ...
            if (!node.transforms.empty()) {
                ...
                if (dt > 1e-6) {
                    ...
                    Eigen::Quaterniond dq =
                        tf.orientation.toEigenQuat() * last_tf.orientation.toEigenQuat().inverse();
    ...
    Transform transform(
        ...
        if (!getTransform(source_frame, target_frame, time, frame_tf)) {
            throw std::runtime_error(
                "Cannot find transform from " + source_frame + " to " + target_frame
            );
        }
...
private:
    ...
    Transform lookupTransformAtTime(const FrameNode& node, const Time& time) const {
        ...
        return cacheResult(
            node,
            time,
            interpolateWithVelocity(it_before->second, it_after->second, alpha, node, time - t0)
        );
    }

    // 插值结果缓存
    Transform cacheResult(const FrameNode& node, const Time& time, const Transform& result) const {
        // 非const访问需要mutable
    ...
    // 带速度模型的插值实现
    Transform interpolateWithVelocity(
        const Transform& a,
        const Transform& b,
        double alpha,
        const FrameNode& node,
        Duration time_since_a
    ) const {
        // 基础插值（位置和旋转）
        ...
        if (!node.linear_velocity.isZero() || !node.angular_velocity.isZero()) {
            ...
            if (!node.linear_velocity.isZero()) {
                base.position =
                    Position::fromEigen(base.position.toEigen() + node.linear_velocity * dt_alpha);
            }
            ...
            if (!node.angular_velocity.isZero()) {
                ...
                if (angle > 1e-6) {
                    ...
                    base.orientation = Quaternion(base_q.x(), base_q.y(), base_q.z(), base_q.w());
                }
            }
        }
    ...
    Transform interpolate(const Transform& a, const Transform& b, double alpha) const {
        // 位置线性插值
        Eigen::Vector3d pos = (1.0 - alpha) * a.position.toEigen() + alpha * b.position.toEigen();
...
inline void quatPosToRTvec(const Quaternion& q, const Position& pos, cv::Mat& rvec, cv::Mat& tvec) {
    // 1. 得到旋转矩阵 (cv::Matx33d)
...
template<>
struct fmt::formatter<tf::Position> {
...
template<>
struct fmt::formatter<std::vector<tf::Quaternion>>: fmt::formatter<std::string_view> {
    template<typename FormatContext>
...
template<>
struct fmt::formatter<tf::Quaternion> {
    constexpr auto parse(format_parse_context& ctx) {
        return ctx.begin();
    }

...
public:
  ...
  Eigen::Vector3d solveBa_t(const ArmorObject &armor,
...
public:
  ...
  EdgeSymmetry(const Sophus::SO3d &R_cam_imu, const Sophus::SO3d &R_pitch,
               const Eigen::Vector3d &t_cam_arm, const Eigen::Matrix3d &K,
               const Eigen::Vector3d &P1, const Eigen::Vector3d &P2,
               const Eigen::Vector2d &meas, double weight = 1.0)
      : R_cam_imu_(R_cam_imu), R_pitch_(R_pitch), t_cam_arm_(t_cam_arm), K_(K),
        P1_(P1), P2_(P2), weight_(weight) {
    setMeasurement(meas);
...
// 2) 仅优化 tvec 的投影边
class EdgeProjectionTvecOnly
    : public g2o::BaseUnaryEdge<2, Eigen::Vector2d, VertexTranslation> {
public:
  EIGEN_MAKE_ALIGNED_OPERATOR_NEW
  EdgeProjectionTvecOnly(const Sophus::SO3d &R_fixed, const Eigen::Vector3d &Xw,
                         const Eigen::Matrix3d &K)
      : R_fixed_(R_fixed), Xw_(Xw), K_(K) {}
...
// 3) 仅优化 tvec 的对称误差边
class EdgeSymmetryTvecOnly
    : public g2o::BaseUnaryEdge<2, Eigen::Vector2d, VertexTranslation> {
public:
  EIGEN_MAKE_ALIGNED_OPERATOR_NEW
  EdgeSymmetryTvecOnly(const Sophus::SO3d &R_fixed, const Eigen::Vector3d &P1,
                       const Eigen::Vector3d &P2,
                       const Eigen::Vector2d &measCenter,
                       const Eigen::Matrix3d &K, double weight = 1.0)
      : R_fixed_(R_fixed), P1_(P1), P2_(P2), K_(K) {

...
OneYpdTracker::OneYpdTracker(double max_match_distance,
                             ...
                             double max_match_x_diff, double jump_thresh)
...
double OneYpdTracker::orientationToYaw(const tf::Quaternion &q) noexcept {
  double roll, pitch, yaw;
  tf::Matrix3x3(q).getRPY(roll, pitch, yaw);
  yaw =
      this->last_yaw_ + angles::shortest_angular_distance(this->last_yaw_, yaw);
  this->last_yaw_ = yaw;
  return yaw;
}

...
TrackerManager::TrackerManager(const YAML::Node &config) {
  track_one_num = config["track_one_num"].as<int>(2);
  // Tracker 基础参数
  ...
  one_tracker_->tracking_thres = config["tracking_thres"].as<int>(5);
  ypd_tracker_->tracking_thres = config["tracking_thres"].as<int>(5);
  one_ypd_tracker_->tracking_thres = config["tracking_thres"].as<int>(5);
  lost_time_thres_ = config["lost_time_thres"].as<double>(0.3);
  one_lost_time_thres_ = config["one_lost_time_thres"].as<double>(0.1);
  ...
  ys2qx_ = config["ekf"]["ys2qx"].as<double>(20.0);
  ys2qy_ = config["ekf"]["ys2qy"].as<double>(20.0);
  ys2qz_ = config["ekf"]["ys2qz"].as<double>(20.0);
  ys2qyaw_ = config["ekf"]["ys2qyaw"].as<double>(100.0);
  ys2qr_ = config["ekf"]["ys2qr"].as<double>(800.0);
  ys2qd_zc_ = config["ekf"]["ys2qd_zc"].as<double>(800.0);

  yr_y_ = config["ekf"]["yr_y"].as<double>(0.05);
  yr_p_ = config["ekf"]["yr_p"].as<double>(0.05);
  yr_d_ = config["ekf"]["yr_d"].as<double>(0.05);
  yr_yaw_ = config["ekf"]["yr_yaw"].as<double>(0.02);

  os2qx_ = config["ekf"]["os2qx"].as<double>(20.0);
  os2qy_ = config["ekf"]["os2qy"].as<double>(20.0);
  os2qz_ = config["ekf"]["os2qz"].as<double>(20.0);
  os2qyaw_ = config["ekf"]["os2qyaw"].as<double>(100.0);

  or_x_ = config["ekf"]["or_x"].as<double>(0.05);
  or_y_ = config["ekf"]["or_y"].as<double>(0.05);
  or_z_ = config["ekf"]["or_z"].as<double>(0.05);
  or_yaw_ = config["ekf"]["or_yaw"].as<double>(0.02);

  oys2qx_ = config["ekf"]["oys2qx"].as<double>(20.0);
  oys2qy_ = config["ekf"]["oys2qy"].as<double>(20.0);
  oys2qz_ = config["ekf"]["oys2qz"].as<double>(20.0);
  oys2qyaw_ = config["ekf"]["oys2qyaw"].as<double>(100.0);

  oyr_y_ = config["ekf"]["oyr_y"].as<double>(0.05);
  oyr_p_ = config["ekf"]["oyr_p"].as<double>(0.05);
  oyr_d_ = config["ekf"]["oyr_d"].as<double>(0.05);
  oyr_yaw_ = config["ekf"]["oyr_yaw"].as<double>(0.02);
  // EKF 状态预测函数

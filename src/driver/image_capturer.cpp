...
public:
    Impl(const CameraProfile& profile, char* user_defined_name, const SyncMode& sync_mode):
        user_defined_name_(user_defined_name),
        profile_(profile),
    ...
    cv::Mat read(std::chrono::duration<unsigned int, std::milli> timeout) {
        ...
        if (converted_data_buffer_ == nullptr) {
            ...
            convert_parameter_.enDstPixelType = PixelType_Gvsp_RGB8_Packed; // 使用 RGB 格式代替 BGR
        ...
        if (ret != MV_OK) {
            WUST_ERROR(hik_logger) << "Failed to convert pixel type. nRet [" << ret << "]";
    ...
    bool software_trigger_on() {
        ...
        if (MV_OK != ret) {
            WUST_ERROR(hik_logger) << "Failed To send software trigger nRet [" << ret << "]";
...
#define SDK_RET_ASSERT(ret, message) \
    ...
    bool print_device_info(MV_CC_DEVICE_INFO* pstMVDevInfo) {
        ...
        if (pstMVDevInfo->nTLayerType == MV_GIGE_DEVICE) {
            ...
            WUST_INFO(hik_logger) << "DeviceIp:" << nIp1 << "." << nIp2 << "." << nIp3 << "."
                                  << nIp4;
            // RCLCPP_INFO(logger_, "DeviceIp: %d.%d.%d.%d", nIp1, nIp2, nIp3, nIp4);
            WUST_INFO(hik_logger) << "UserDefinedName:"
            ...
            //     logger_, "UserDefinedName: %s",
            //     pstMVDevInfo->SpecialInfo.stUsb3VInfo.chUserDefinedName);
            WUST_INFO(hik_logger) << "Serial Number:"
                                  << pstMVDevInfo->SpecialInfo.stUsb3VInfo.chSerialNumber;
            // RCLCPP_INFO(
            //     logger_, "Serial Number: %s",
            //     pstMVDevInfo->SpecialInfo.stUsb3VInfo.chSerialNumber);
            WUST_INFO(hik_logger) << "Device Number:"
    ...
    MV_CC_DEVICE_INFO* search_camera(const char* user_defined_name) {
        ...
        if (user_defined_name == nullptr) {
            if (device_list.nDeviceNum > 1) {
                WUST_ERROR(hik_logger) << device_list.nDeviceNum
    ...
    void uninit_camera() {
        ...
        if (MV_OK != nRet)
            WUST_ERROR(hik_logger) << "Failed to stop grabbing. nRet [" << nRet << "]";
        ...
        if (MV_OK != nRet)
            WUST_ERROR(hik_logger) << "Failed to destroy handle. nRet [" << nRet << "]";

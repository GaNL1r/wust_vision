...
ArmorDetectOpenCV::ArmorDetectOpenCV(const std::string &classify_model_path,
                                     const std::string &classify_label_path,
                                     const int &bin_thres,
                                     const double &classifier_threshold,
                                     const LightParams &l, const ArmorParams &a)
    : binary_thres(bin_thres), light_params(l), armor_params(a),
...
ArmorDetectOpenCV::detect(const cv::Mat &input) noexcept {
  if (input.empty())
    return {};
  cv::Mat gray_img_;
  ...
  std::for_each(std::execution::par, armors.begin(), armors.end(),
                [this, &input, gray_img_, &valid_armors,
                 &valid_mutex](ArmorObject &armor) {
                  try {
                    ...
                    std::cerr << "[detect] Exception: " << e.what()
                              << std::endl;
                  }
                });
...
cv::Mat ArmorDetectOpenCV::preprocessImage(const cv::Mat &rgb_img,
                                           cv::Mat &gray_img_) noexcept {
...
std::vector<Light>
ArmorDetectOpenCV::findLights(const cv::Mat &rgb_img,
...
bool ArmorDetectOpenCV::isLight(const Light &light) noexcept {
  ...
  bool ratio_ok =
      light_params.min_ratio < ratio && ratio < light_params.max_ratio;
...
std::vector<ArmorObject>
ArmorDetectOpenCV::matchLights(const std::vector<Light> &lights) noexcept {
...
bool ArmorDetectOpenCV::containLight(
    const int i, const int j, const std::vector<Light> &lights) noexcept {
...
ArmorType ArmorDetectOpenCV::isArmor(const Light &light_1,
...
cv::Mat
ArmorDetectOpenCV::extractNumber(const cv::Mat &src,
  ...
  cv::Point2f lights_vertices[4] = {armor.lights[0].bottom, armor.lights[0].top,
                                    armor.lights[1].top,

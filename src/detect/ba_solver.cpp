...
Eigen::Matrix3d BaSolver::solveBa(const ArmorObject &armor,
  ...
  auto object_points = ArmorObject::buildObjectPoints<Eigen::Vector3d>(
      armor_size.x(), armor_size.y());
  const auto &landmarks = armor.landmarks();
  ...
  for (auto &p : symPairs) {
    ...
    Eigen::Vector2d measCenter =
        (Eigen::Vector2d(landmarks[i].x, landmarks[i].y) +
         Eigen::Vector2d(landmarks[j].x, landmarks[j].y)) *
        0.5;

    auto *edgeSym = new EdgeSymmetry(R_camera_imu, R_pitch, t_camera_armor, K_,
                                     object_points[i], object_points[j],
                                     measCenter, symWeight);
...
Eigen::Vector3d BaSolver::solveBa_t(
    const ArmorObject &armor, const Eigen::Vector3d &t_camera_armor_init,
    const Eigen::Matrix3d &R_camera_armor, // 已优化的旋转
    const Eigen::Matrix3d & /*unused R_imu_camera*/, int type_number) noexcept {
  optimizer_.clear();
  ...
  const double symWeight = 1.0;
  for (auto &[i, j] : sym) {
    Eigen::Vector2d meas_center =
        0.5 * (Eigen::Vector2d(landmarks[i].x, landmarks[i].y) +
               Eigen::Vector2d(landmarks[j].x, landmarks[j].y));
    auto *es =
        new EdgeSymmetryTvecOnly(R_fixed, object_points[i], object_points[j],
                                 meas_center, K_, symWeight);
    es->setId(id++);

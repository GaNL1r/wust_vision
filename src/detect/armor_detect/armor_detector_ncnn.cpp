#include "detect/armor_detect/armor_detector_ncnn.hpp"
...
void ArmorDetectNCNN::initNumberClassifier() {
    // 加载数字识别模型
...
void ArmorDetectNCNN::extractNumberImage(const cv::Mat& src, ArmorObject& armor) {
    // 光条长度和装甲板尺寸参数
...
bool ArmorDetectNCNN::refineLightsFromArmorPts(ArmorObject& armor) const {
...
std::vector<Light> ArmorDetectNCNN::findLights(
...
bool ArmorDetectNCNN::isLight(const Light& light) noexcept {
...
bool ArmorDetectNCNN::isArmor(const Light& light_1, const Light& light_2) noexcept {
...
void ArmorDetectNCNN::detect(ArmorObject& armor) {
...
bool ArmorDetectNCNN::classifyNumber(ArmorObject& armor) {

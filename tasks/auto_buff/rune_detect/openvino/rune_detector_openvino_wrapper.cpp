...
#include "rune_detector_openvino_wrapper.hpp"
#include <wust_vl/common/utils/logger.hpp>

RuneDetectorOpenvinoWrapper::RuneDetectorOpenvinoWrapper(const YAML::Node& config) {
    std::string model_path = config["rune_detector"]["model"].as<std::string>();
    std::string device_type = config["rune_detector"]["device_type"].as<std::string>("CPU");
...
std::tuple<cv::Point2f, cv::Mat> RuneDetectorOpenvinoWrapper::detectRTag(
    const cv::Mat& img,
    int binary_thresh,

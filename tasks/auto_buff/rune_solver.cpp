...
RuneSolver::RuneSolver(const YAML::Node& config, std::pair<cv::Mat, cv::Mat> camera_info) {
    camera_info_ = camera_info;
    // Init
    rune_solver_params_ = RuneSolver::RuneSolverParams {
        .compensator_type = config["rune_solver"]["compensator_type"].as<std::string>(),
        .gravity = config["rune_solver"]["gravity"].as<double>(9.8),
        .angle_offset_thres = config["rune_solver"]["angle_offset_thres"].as<double>(0.78),
        .lost_time_thres = config["rune_solver"]["lost_time_thres"].as<double>(0.5),
        .auto_type_determined = config["rune_solver"]["auto_type_determined"].as<bool>(true),
    };
    tracker_state = LOST;
    ...
    predict_offset_ = config["rune_solver"]["predict_offset"].as<double>(0.0);
    ...
    if (config["rune_solver"]["trajectory_offset"]) {
        for (const auto& node: config["rune_solver"]["trajectory_offset"]) {
            OffsetEntry e;
    ...
    yq_vec_ = config["rune_solver"]["ekf"]["q_ypdyaw"].as<std::vector<double>>();
    auto yu_q = [this]() {
    ...
    yr_vec_ = config["rune_solver"]["ekf"]["r_ypdyaw"].as<std::vector<double>>();
    ...
    iteration_num_ = config["rune_solver"]["ekf"]["iteration_num"].as<int>(1);
...
GimbalCmd RuneSolver::solveGimbalCmd(const Eigen::Vector3d& target) {
    ...
    current_pitch = state_result_.rpy[1];
    current_yaw = state_result_.rpy[2];
    ...
    if (double temp_pitch = pitch;
        trajectory_compensator_->compensate(target, temp_pitch, state_result_.bullet_speed))
    {
        pitch = temp_pitch;
    }
...
GimbalCmd RuneSolver::solve() {
    ...
    state_result_ = state_callback_();
    // Calculate predict time
    Eigen::Vector3d cur_pos = getTargetPosition(0);
    double flying_time =
        trajectory_compensator_->getFlyingTime(cur_pos, state_result_.bullet_speed);

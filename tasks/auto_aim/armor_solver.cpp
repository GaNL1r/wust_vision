...
void ArmorSolver::init(const YAML::Node& config) {
    ...
    armor_slover_state_ = ArmorSloveState::TRACKING_ARMOR;
    overflow_count_ = 0;
    transfer_thresh_ = 5;
}
...
GimbalCmd ArmorSolver::solve(
    ...
    state_result_ = state_callback_(armor_slover_state_);
    rpy = state_result_.rpy;
    double controller_delay = state_result_.controller_delay;
    // 2. 选择最优单目标索引
    ...
    if (use_multi) {
        ...
        double fly_t = trajectory_compensator_->getFlyingTime(pos, state_result_.bullet_speed);
        double dt_sec =
        ...
        switch (armor_slover_state_) {
            case ArmorSloveState::TRACKING_ARMOR:
                ...
                if (overflow_count_ > transfer_thresh_) {
                    armor_slover_state_ = ArmorSloveState::TRACKING_CENTER;
                    overflow_count_ = 0;
                }
                chosen = armors[idx];
            ...
            case ArmorSloveState::TRACKING_CENTER:
                if (std::abs(target.v_yaw) < max_tracking_v_yaw_) {
                    ++overflow_count_;
                } else {
                    overflow_count_ = 0;
...
std::vector<std::pair<double, double>> ArmorSolver::getTrajectory() const noexcept {
    auto traj = trajectory_compensator_->getTrajectory(15, rpy_[1], state_result_.bullet_speed);
    for (auto& p: traj) {
...
void ArmorSolver::calcYawAndPitch(
    ...
    if (double temp_pitch = pitch;
        trajectory_compensator_->compensate(p, temp_pitch, state_result_.bullet_speed))
    {
        pitch = temp_pitch;
    }
}

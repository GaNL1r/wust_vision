...
public:
    ...
    bool extractNetImage(const cv::Mat& src, ArmorObject& armor) const {
        ...
        for (auto& armor_in: objs_result) {
            ...
            try {
                ...
                try {
                    ...
                    std::cerr << "[detectNet] exception in classifyNumber: " << e.what()
                              << std::endl;
                    continue;
                } catch (...) {
                ...
                std::cerr << "[ArmorDetectorCommon::detectNet] something failed (unexpected)."
                          << std::endl;
            }
        }
...
std::vector<ArmorObject> ArmorDetectorCommon::detectNet(
    ...
    return _impl->detectNet(src_img, objs_result, transform_matrix, detect_color, target_number);
}
} // namespace auto_aim
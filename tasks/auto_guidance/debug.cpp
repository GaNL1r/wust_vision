...
void drawAutoGuidanceDebugContent(cv::Mat& debug_img, const AutoGuidanceDebug& dbg) {
    ...
    double diff_center_norm = (target.is_tracking_) ? cx_norm : 0;
    {
        ...
        int baseline = 0;
        cv::Size text_size = cv::getTextSize(diff_str, font_face, font_scale, thickness, &baseline);
        ...
        cv::rectangle(
            debug_img,
            cv::Rect(x - 5, y - text_size.height - 5, text_size.width + 10, text_size.height + 10),
            cv::Scalar(0, 0, 0),
            cv::FILLED,
            cv::LINE_AA
        );
    ...
    if (target.is_tracking_) {
        ...
        std::string line1 = fmt::format("pos: {:.1f} {:.1f} {:.1f} {:.1f}", s(0), s(2), s(4), s(6));
        std::string line2 = fmt::format("vel: {:.2f} {:.2f} {:.2f} {:.2f}", s(1), s(3), s(5), s(7));

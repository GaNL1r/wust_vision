...
def calculate_trajectory(
    angle_deg, v0, distance, height_diff, drag_coefficient=0, mass=1
):
    """
    ...
    def update_plot(self):
        ...
        if angles is None:
            ...
            self.ax.text(
                0.5,
                0.5,
                "No valid solution. Please adjust parameters.",
                ha="center",
                va="center",
                fontsize=14,
                color="red",
        ...
        if drag > 0:
            x, y = calculate_trajectory(
                angles[0], velocity, distance, height_diff, drag_coefficient=drag
            )
            self.ax.plot(x, y, "b-", label=f"With Drag: {angles[0]:.1f}°")
            all_y = y
        else:
            x1, y1 = calculate_trajectory_no_drag(
                angles[0], distance, height_diff, velocity
            )
            x2, y2 = calculate_trajectory_no_drag(
                angles[1], distance, height_diff, velocity
            )

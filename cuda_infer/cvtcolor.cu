...
void storeRGB(uint8_t* out, int R, int G, int B) {
...
__global__ void bayer_kernel(
    ...
    constexpr bool G_on_R_row_pattern =
        (P == BayerPattern::RGGB || P == BayerPattern::GRBG);
...
void CudaBayer_EA::allocIfNeeded(int width, int height) {
    ...
    if (need_bayer <= bayer_capacity_ &&
        need_rgb   <= rgb_capacity_) {
        width_  = width;
        height_ = height;
        return;
    }
